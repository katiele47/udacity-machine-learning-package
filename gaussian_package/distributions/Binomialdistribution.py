...
class Binomial(Distribution):
    ...
    #           the probability of getting a positive outcome --> p
    ...
    def plot_bar(self):
        ...
        plt.hist(x, y)
        plt.title('Histogram or Bar chart whatever')
        plt.xlabel('Values')
        plt.ylabel('Counts')
    ...
    def pdf(self, k):
        ...
        n = self.n
        p = self.p
        pdf = (math.factorial(n)/matgh.factorial(k)*math.factorial(n-k))*(p**k)*((1-p)**(n-k))
    ...
    def plot_bar_pdf(self):
        ...
        x = []
        y = []
        for i in range(self.n + 1): #include n+1 
            x.append(i)
            y.append(self.pdf(i))
